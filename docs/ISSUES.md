...
| ISSUE-058 | bug | Sucesso silencioso: webhook `state=success` com `occ_command_failed` embutido no summary projeta read-model falso (users/groups/saga); grupos de template não validados | Jobs, Webhook, Customers, Onboarding, Occ | HIGH | open — findings ARQ-B1..B3, ARQ-A5 |
