...
class Create extends Component
{
    ...
    public function save(
        ...
        $cluster = DB::transaction(function () use ($pem, $sftpPem, $plainSecret) {
            ...
            $created->ssh_private_key_encrypted = $pem;
            $created->webhook_secret_encrypted = $plainSecret;
            if ($sftpPem !== '') {
                $created->sftp_private_key_encrypted = $sftpPem;
            }
            $created->save();

            $history = WebhookSecretHistory::createWithSecret([
